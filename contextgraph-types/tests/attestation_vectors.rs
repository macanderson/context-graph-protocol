...
// Deliberately NOT `#![cfg(feature = "attestation")]`.
//
...
// ran nothing (#117). These vectors pin the wire format for provenance attestation (`SPEC.md` §6.5); they are
...
fn these_vectors_are_only_meaningful_with_the_feature_on() {
    assert!(
        ...
        "this suite pins wire-format bytes and was compiled WITHOUT the \
         `attestation` feature, so none of its vectors ran. Build it with \
         `cargo test -p contextgraph-types --features attestation`. Failing \
