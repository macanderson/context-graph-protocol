...
The canonical records are context records in the oxagen workspace,
...
This repository is linked to that workspace and does not carry a copy.
The record is [ADR 0020](docs/adr/0020-standing-decisions-are-workspace-records.md).
...
- **[SCR-006](https://github.com/macanderson/oxagen/blob/main/.oxagen/rules/ctx.scr.006-schema-change-labelled.toml) — Schema
  ...
  has nothing to label. Here: this repository has no persistent store, so the
  record is inert and the label is never applied. A change to `schema/*.json`
  is a wire change and takes the protocol-stability section of the pull
  request template, not this label.
