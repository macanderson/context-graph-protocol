---
title: "Conformance registry"
description: "Providers that are Context Graph Protocol conformant, with a reproducible report backing each claim, and how to get your own provider listed."
---
...
[running-conformance.md](./running-conformance)) — with a reproducible,
...
([governance.md](./governance#the-path-to-contextgraph10)). This registry is
...
1. **Run the suite against your provider** with `contextgraph-inspect ... --json`
   (see [running-conformance.md](./running-conformance)) and confirm every
...
![CGP conformant](/badges/conformant.svg)
...
The URL is the spec repository's GitHub-raw path rather than a
...
does. See ADR 0008 for the host boundary and
[#57](https://github.com/macanderson/context-graph-protocol/issues/57) for the
deploy topology this depends on.
