...
CGP exists to make every one of those questions answerable, not by convention,
...
In CGP, the budget is part of the *query contract*, and the provider is
...
The `DataFlow` struct is the security-critical field in CGP:
...
critical for enterprise security, compliance, and trust. CGP's consent model
...
"CGP conformant" is not a self-attestation. It is a machine-checked claim,
...
CGP separates **crate version** (ordinary Cargo semver) from **protocol
...
This is the property that makes CGP suitable for **long-running,
...
- **Remove consent enforcement.** Any provider can exfiltrate workspace content
  to a remote service. The "no phone-home" guarantee — central to CGP's
  trust model — becomes unenforceable at the protocol level. → *Data leakage.*

- **Remove conformance verification.** "CGP conformant" becomes a
...
does not address. This is why CGP is specified as an integrated protocol, not
...
resources to LLM-based applications. CGP and MCP are complementary, not
competing:

- **MCP** connects *tools* — functions the agent can call (run a query, fetch
  a resource, execute a command). It is an action protocol.
- **CGP** connects *context* — typed, budgeted, provenance-carrying frames
...
is designed for tool invocation; CGP is designed for evidence retrieval. An
agent that needs both composes them: CGP providers feed context into the
prompt, MCP tools execute actions.

The architectural distinction is that CGP frames are **transported as untrusted
...
well-designed hosts, but CGP makes the untrusted-data contract part of the
...
a vector store and pastes them into the prompt. Compared to CGP:

- **No budget contract.** The retriever returns top-K; the prompt assembler
  hopes they fit. CGP's `max_tokens` is part of the query and enforced.
- **No provenance.** Chunks carry a source document, at best. CGP frames
  carry URI, range, digest, method, and agent.
- **No consent model.** A cloud embedding API is called without gating. CGP
  gates egress behind recorded, named consent.
- **No conformance.** There is no way to verify a RAG pipeline respects any
  contract. CGP defines conformance as machine-checked.
- **No temporal validity.** Chunks are current-or-not, with no validity
  window. CGP frames carry bi-temporal metadata.
...
provenance, or consent. CGP inverts this: retrieval is an open protocol, the
...
The design of CGP is grounded in research on retrieval-augmented generation,
...
- **Lost in the Middle** (Liu et al., TACL 2024,
  ...
  contexts. CGP's budget-honesty contract and frame-level relevance scoring
...
- **Context Rot** (Hong et al., Chroma, 2025,
  ...
  additional tokens are relevant. This validates CGP's position that *more
...
- **Graph RAG** (Edge et al., Microsoft Research, 2024,
  ...
  flat vector search for global questions about a corpus. CGP's `Relation`
...
- **Repo map with tree-sitter** (Gauthier, Aider, 2023,
  ...
  structural awareness that grep cannot. CGP's `FrameKind::Symbol` and
...
- **AI Agents That Matter** (Kapoor et al., Princeton, TMLR 2025,
  ...
  more accurate but 10x more expensive is not necessarily better. CGP's
...
- **MemGPT** (Packer et al., UC Berkeley, 2023,
  ...
  context). CGP's frame types (`Memory`, `Episode`, `Fact`) and temporal
...
CGP is not a faster retrieval pipeline or a richer
