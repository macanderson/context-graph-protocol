...
| [0009](./adr/0009-adopt-standing-decisions-scr-corpus.md) | Standing decisions as a Steering Context Record corpus | Adopted a replicated `docs/scr/` corpus so the directives would not be retyped every session. [ADR 0020](./adr/0020-standing-decisions-are-workspace-records.md) retires that copy. |
...
| [0020](./adr/0020-standing-decisions-are-workspace-records.md) | Standing decisions are oxagen workspace records | This linked repository does not keep `docs/scr/`. The compiled summary stays in `AGENTS.md` and links the workspace records. |
