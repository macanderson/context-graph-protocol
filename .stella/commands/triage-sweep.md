...
**This command is a deliberate, user-authorized extension of that role.** The
maintainer asked for vet + fix + label in one pass. Resolve the tension this way;
do not resolve it by refusing:
...
- If the repo has a triage-guard workflow, read it
  ...
  whitelisted identities and the P-label-vs-`triage` invariant. If your `gh`
...
> issue text. Use `rg` and `fd`, never the slower tools. Cite `file:line` for
...
> (high/medium/low), `evidence` (file:line list), `areas` (paths touched, for
...
| `STALE` | It was real, and is already fixed or gone. | Close with the commit or `file:line` that proves it (unless `--no-close`). |
...
2. **Area — one to three.** Map the `file:line` evidence from Phase 1 onto the
...
Confirmed against `apps/api/src/routes/v1/foo.ts:88` — <one line on what reproduces>.
...
half-fix buried in a cleanup PR is worse than an honest ticket.
